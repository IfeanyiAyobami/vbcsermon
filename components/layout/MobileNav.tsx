  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-white/10 bg-[var(--vbc-panel)]/95 backdrop-blur-xl lg:hidden" style={{paddingBottom:"env(safe-area-inset-bottom)"}}>
      {navLinks.map(l=>{
        ...
        const Icon=l.icon;
        return (
          <Link key={l.href} href={l.href} target={l.external?"_blank":undefined} rel={l.external?"noopener noreferrer":undefined} className="flex flex-1 flex-col items-center gap-1 py-3">
